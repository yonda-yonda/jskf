...
const sweepOutMethod = (mat: Matrix, e?: number) => {
  /**
   *    掃き出し法による逆行列と行列式を求める。
   */
...
class Matrix {
  ...
  splice(
    ...
    if (direction === "row") {
      ...
      if (refillIndex === 0) this.dimention[1] = 0;
    } else {
      ...
      if (refillIndex === 0) this.dimention[0] = 0;
    }
  ...
  ope(operator: "+" | "-" | "*" | "/", val: number | Matrix): Matrix {
    ...
    if (typeof val === "number")
      ...
      if (
        this.dimention[0] !== val.dimention[0] ||
  ...
  row(index: number): Vector {
    ...
    return new Vector(this.values[row].slice());
  }
  ...
  static eye(row: number, col?: number, pos?: number) {
    ...
    if (position >= 0) {
      ...
      if (position > mat.dimention[1]) position = mat.dimention[1];
      for (let i = 0; i < size; i++) {
        if (
          typeof mat.values[i] === "undefined" ||
      ...
      position *= -1;
      position = Math.floor(position);
      if (position > mat.dimention[0]) position = mat.dimention[0];
      for (let i = 0; i < size; i++) {
        if (
          typeof mat.values[i + position] === "undefined" ||
