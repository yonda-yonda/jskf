...
function randomStr(length: number = 8) {
...
export const show = function (
  ...
  const marginLeft = margin?.left && margin.left > 0 ? margin.left : 60;
  const marginRight = margin?.right && margin.right > 0 ? margin.right : 30;
  const marginTop = margin?.top && margin.top > 0 ? margin.top : 30;
  const marginBottom = margin?.bottom && margin.bottom > 0 ? margin.bottom : 30;
  const innerWidth = svgWidth - marginLeft - marginRight;
  const innerHeight = svgHeight - marginTop - marginBottom;
  const startIndex =
    typeof start !== "undefined" && start > 0 ? Math.floor(start) : 0;
  const endIndex =
    typeof end !== "undefined" && startIndex < end
      ? Math.floor(end)
  ...
  const graphColors = Array.isArray(colors) ? colors : schemeSet1;

  const svg = select(elm).append("svg");
  if (resize) svg.attr("viewBox", [0, 0, svgWidth, svgHeight].toString());
  else svg.attr("width", svgWidth).attr("height", svgHeight);
  ...
  let observedValues: (number[] | null)[] = [];
  ...
  for (let step = 0; step < endIndex - startIndex; step++) {
    ...
    if (Array.isArray(observedValues[step])) {
      observedIndexArr.forEach((index) => {
        const y = (observedValues[step] as number[])[index];
    ...
    if (typeof stateValues[step] !== "undefined") {
      for (const [index, area] of zip(stateIndexArr, varianceIndexArr)) {
        ...
        if (typeof x !== "undefined") {
          ...
          if (area) {
            ...
            if (typeof v2 !== "undefined") {
              item.upper.push(x + 1.96 * v2 ** 0.5);
              item.lower.push(x - 1.96 * v2 ** 0.5);
            }
          }
        }
      }
  ...
  frame
    ...
    .call(
      axisBottom(xScale)
        .tickValues(
          Array.from(range(startIndex, endIndex)).filter((d, i) => {
        ...
        .tickFormat(function (d, i) {
          ...
          if (!ticks) {
            return String(value);
          } else if (ticks[value]) return String(ticks[value]);
          else return "";
        })
    );
  const yScale = scaleLinear().domain(yRange).range([innerHeight, 0]);
  frame.append("g").call(axisLeft(yScale));
  ...
  for (let i = 0; i < stateIndexArr.length; i++) {
    if (varianceIndexArr[i])
      frame
        ...
        .attr(
          "fill",
          graphColors[(i + observedIndexArr.length) % graphColors.length]
        )
  ...
  const stateLegends = stateIndexArr.map((order, index) => {
    return {
      label: `state[${order}]`,
      color:
        graphColors[(index + observedIndexArr.length) % graphColors.length],
    };
  });
